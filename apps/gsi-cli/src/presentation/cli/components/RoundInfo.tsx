...
export function RoundInfo({ round }: RoundInfoProps) {
  ...
  return (
    <Box flexDirection="column">
      <Box>
        <Text bold color="cyan">Round {round.roundNumber}</Text>
        <Text color="gray"> • </Text>
        <Text color={teamColor} bold>{round.playerTeam}</Text>
        {round.winnerTeam && (
          <>
            <Text color="gray"> • Winner: </Text>
            <Text color={round.winnerTeam === 'CT' ? 'blue' : 'red'} bold>{round.winnerTeam}</Text>
          </>
        )}
      </Box>
      ...
      {round.weaponTransactions.length > 0 && (
        <Box>
          ...
          {round.weaponTransactions
            ...
            .map((t, i: number) => (
              <Text key={i} color="white">{i > 0 ? ', ' : ''}{t.weaponName} <Text color="yellow">${t.cost}</Text></Text>
            ))
          }
        </Box>
      )}
