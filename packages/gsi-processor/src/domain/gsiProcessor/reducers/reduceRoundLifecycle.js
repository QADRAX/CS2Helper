...
function shouldCreateRound(ctx, currentRoundExists) {
    const { memory, snapshot } = ctx;
    if (!snapshot.round)
        return false;
    return (!currentRoundExists &&
...
function markRoundLive(ctx, roundNumber) {
...
function markPreviousRoundOver(ctx) {
...
function assignRoundWinner(ctx, winnerTeam) {
    const { state, memory, timestamp, events } = ctx;
    if (!state.currentMatch)
        return;
    const lastRound = findRound(state.currentMatch, memory.lastGameRound);
...
export function reduceRoundLifecycle(ctx) {
    ...
    if (shouldCreateRound(ctx, hasCurrentRound))
        createRound(ctx, gameRound);
    // `round_live` belongs to the current round number, not the previous one.
    if (findRound(state.currentMatch, gameRound) &&
        memory.lastRoundPhase === "freezetime" &&
        roundPhase === "live") {
        markRoundLive(ctx, gameRound);
    }
    if (findRound(state.currentMatch, memory.lastGameRound) &&
