...
function canReducePlayerEvents(ctx) {
 ...
 */
function createPreviousPlayerMemory(frame) {
    return {
        health: frame.health,
        kills: frame.kills,
        deaths: frame.deaths,
...
function pushKillEvent(ctx, frame) {
...
function pushDeathEvent(ctx, frame) {
    const { state, timestamp, events } = ctx;
    if (!state.currentMatch)
        return;
    const round = findRound(state.currentMatch, frame.eventRound);
...
function pushDamageEvent(ctx, frame, damage) {
    const { state, timestamp, events } = ctx;
    if (!state.currentMatch)
        return;
 ...
 */
function reduceFlashEvents(ctx, frame) {
    const { state, timestamp, events } = ctx;
    if (!state.currentMatch)
        return;
...
function pushWeaponTransaction(ctx, frame, transactionType, weaponName, cost) {
 ...
 */
function reduceWeaponTransactions(ctx, frame) {
    if (frame.gameRound !== ctx.memory.lastGameRound || !ctx.state.currentMatch)
        return;
...
function persistPlayerMemory(ctx, frame) {
 ...
 */
export function reducePlayerEvents(ctx) {
    const { memory, snapshot } = ctx;
    if (!canReducePlayerEvents(ctx) || !snapshot.map || !snapshot.round)
        return;
    ...
    for (const player of snapshot.players) {
        ...
        if (frame.player.kills > frame.previous.kills)
            pushKillEvent(ctx, frame);
        if (frame.player.deaths > frame.previous.deaths)
            pushDeathEvent(ctx, frame);
        const damage = frame.previous.health - frame.player.health;
        if (damage > 0)
            pushDamageEvent(ctx, frame, damage);
