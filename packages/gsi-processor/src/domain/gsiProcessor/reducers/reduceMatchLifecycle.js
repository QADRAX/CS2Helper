 ...
 */
function shouldStartMatch(ctx) {
    const { state, memory, snapshot } = ctx;
    if (state.currentMatch || !snapshot.map)
        return false;
    return ((snapshot.map.phase === "warmup" && memory.lastMapPhase !== "warmup") ||
 ...
 */
export function reduceMatchLifecycle(ctx) {
    const { snapshot } = ctx;
    if (!snapshot.map)
        return;
    if (shouldStartMatch(ctx))
        startMatch(ctx);
    if (shouldEndMatch(ctx))
        endMatch(ctx);
}
